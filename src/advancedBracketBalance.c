...
int main()
{
    // подразумеватся, что пользователь может ввести не только скобки и в кол-ве символов он должен указать
    // всё кол-во символов включая скобки и не скобки.
    int len = 0;
    ...
    for (int i = 0; i < len; i++) {
        ...
        if ((tmp == 40) || (tmp == 41) || (tmp == 91) || (tmp == 93) || (tmp == 123) || (tmp == 125)) {
